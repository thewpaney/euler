/* 
 * Problem 11: Larget product in a grid
...
fn main() {
    ...
    let string_length: usize = 4;  // Check this many adjacent numbers
    let mut r: usize = 0;
    let mut c: usize = 0;
    let mut max: u32 = 0;
    while r < grid.len() - string_length + 1 {
	while c < grid[r].len() - string_length + 1 {
	    // Four things to check: down (row inc), right (col inc), and diagonal (row inc + col inc OR row inc + col dec)
	    let mut down_product: u32 = 1;
	    let mut right_product: u32 = 1;
	    let mut diag_left_product: u32 = 1;
	    let mut diag_right_product: u32 = 1;
	    for string_idx in 0..string_length {
		down_product *= grid[r + string_idx][c];
		right_product *= grid[r][c + string_idx];
		diag_right_product *= grid[r + string_idx][c + string_idx];
		if c >= string_length - 1 {
		    diag_left_product *= grid[r + string_idx][c - string_idx];
		}
	    }
	    if down_product > max {
		println!("New max DOWN from ({}, {})", r, c);
		max = down_product;
	    }
	    if right_product > max {
		println!("New max RIGHT from ({}, {})", r, c);
		max = right_product;
	    }
	    if diag_left_product > max {
		println!("New max DIAG LEFT from ({}, {})", r, c);
		max = diag_left_product;
	    }
	    if diag_right_product > max {
		println!("New max DIAG RIGHT from ({}, {})", r, c);
		max = diag_right_product;
	    }
	    // NEXT
	    c += 1;
	}
